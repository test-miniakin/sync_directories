    ...
    def compare_nodes(self):
        ...
        for node in self.node_list:            
            if self.node_list.index(node) < len(self.node_list) - 1: 
                ...
                print ('\n Comparing Node ' + str(self.node_list.index(node)+1) + ' and Node ' + str(self.node_list.index(node) + 2) + ':')                
    ...
    def _compare_directories(self, left, right):
        ...
        if result.common_dirs:
            for res in result.common_dirs:
                self._compare_directories(os.path.join(left, res), os.path.join(right, res))
        if result.left_only:
        ...
        if result.right_only:
            for res in result.right_only:                
                ...
                if p.is_dir():                    
                    ...
                    logging.info('Deleted directory \"' + os.path.basename(res) + '\" from \"' + os.path.join(right))                    
                else:
                    os.remove(os.path.join(right, res))
                    self.files_deleted_total = self.files_deleted_total + 1
                    logging.info('Deleted \"' + os.path.basename(res) + '\" from \"' + os.path.join(right))                    
        left_newer = []
        right_newer = []
    ...
    def _copy(self, files_list, src, dest):
        ...
        for f in files_list:
            ...
            if os.path.isdir(srcpath):
                ...
                logging.info('Copied directory \"' + os.path.basename(srcpath) + '\" from \"' + os.path.dirname(srcpath) + '\" to \"' + dest + '\"')
            else:
